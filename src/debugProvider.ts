...
exports.DlvDebugProvider = void 0;
const vscode = require("vscode");
const cp = require("child_process");
const path = require("path");
const fs = require("fs");
class DlvDebugProvider {
    ...
    async buildAndDeploy(config) {
        ...
        if (Array.isArray(env) && env.length > 0) {
            envString = env.map(e => `${e}`).join(' ');
        }
        if (envString) {
            envString += ' && ';
        }
        let execCommand = `${serverpath}/${servername}`;
        if (args) {
            execCommand += ` -- ${args}`;
        }
        // 构建命令
        const buildCmd = `cd ${projectPath} && GOOS=linux GOARCH=amd64 go build ${buildArgs} -o ${servername} ${cwd}`;
        // 部署命令
        const deployCmd = `ssh -o "User=root" ${host} "rm -f ${projectPath}/${servername}" && ` +
            ...
            `ssh -o "User=root" ${host} "${envString} nohup /usr/local/bin/dlv --listen=:${port} --headless=true --api-version=2 --accept-multiclient exec ${execCommand}> /dev/null 2>&1 &"`;
        ...
        return new Promise((resolve) => {
            ...
            cp.exec(fullCmd, { cwd: workspaceFolder.uri.fsPath }, async (error, stdout, stderr) => {
                if (error) {
    ...
    // 添加等待dlv启动的方法
    async waitForDlv(host, port, maxRetries) {
    ...
    async cleanupRemote(config) {
        ...
        cp.exec(cleanupCmd, (error, stdout, stderr) => {
            if (error) {
...
exports.DlvDebugProvider = DlvDebugProvider;
